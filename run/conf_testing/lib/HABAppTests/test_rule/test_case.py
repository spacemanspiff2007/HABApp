...
from HABApp.core.internals import EventBusListener, ExecutorFactory, FunctionExecutorBase, get_current_context
...
class ExecutionEventCatcher:
    ...
    async def __aenter__(self) -> Self:
        ...
        if self._listener is None:
            ebl = EventBusListener(self._event_bus_name, context.executor_factory.create(self._event), NoEventFilter())
            self._listener = ebl
...
def tc_wrap_func(func: Callable | Callable[[...], Coroutine], res: TestResult,
                 factory: ExecutorFactory) -> FunctionExecutorBase:
    ...
    if iscoroutinefunction(func):
        ...
        return factory.create(tc_async_wrapped)
    ...
    return factory.create(tc_wrapped)
...
class TestCase:
    ...
    async def run(self, res: TestResult) -> TestResult:
        ...
        executor_factory: Final = get_current_context().executor_factory
        ...
        async with ExecutionEventCatcher('warning', HABApp.core.const.topics.TOPIC_WARNINGS) as worker_warnings, \
            ...
            try:
                async with RestPatcher(name), MqttPatcher(name), WebsocketPatcher(name):
                    if s := self.set_up:
                        await tc_wrap_func(s, res, executor_factory).execute()
                        await sleep(0.1)
                        b.log('Setup done')

                    ret = await tc_wrap_func(self.func, res, executor_factory).execute(*self.args, **self.kwargs)
                    if ret:
            ...
            try:
                if t := self.tear_down:
                    b.log('Tear down')
                    await tc_wrap_func(t, res, executor_factory).execute()
                    await sleep(0.1)
            except Exception as e:
                res.exception(e)
