...
class WrappedFunction:
    ...
    def __init__(self, func, logger=None, warn_too_long = True):
    ...
    def __execute(self, *args, **kwargs):
        ...
        if __start - self.__submitted > 0.05:
            self.log.warning(f'Starting of {self.__func.__name__} took too long: {__start - self.__submitted:.1f}s. '
        ...
        if self.__warn_too_long and __dur > 0.8:
            self.log.warning(f'Execution of {rule_instance.rule_name}.{func.__name__} took too long: {__dur:.1f}s')
